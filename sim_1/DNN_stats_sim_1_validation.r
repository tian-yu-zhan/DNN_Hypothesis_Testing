...
## R code for validation to reproduce results of "DNN" in the third column 
## in Table 1 of Section 4.1 with pre-trained DNNs
...
n.test.MMD.outer.itt = 10^4 # simulation iterations for MMD

###################################################################################
## first DNN on test statistics
first.DNN.model = load_model_hdf5("sim_1/sim_1_first_DNN", 
...
second.DNN.model = load_model_hdf5("sim_1/sim_1_second_DNN", 
                                  custom_objects = NULL, compile = TRUE)
## get the DNN weights
...
load(file = "sim_1/sim_1_scale_parameters")
...
val.para.grid$DNN_validation_hour = val.para.grid$LRT_power = val.para.grid$MMD_power = 
...
for (val.ind in c(1:n.val.ind)){
  ...
  validation.mat = t(sapply(1:n.test.H0.outer.itt, function(x){
    ...
    data.val.fit = get.data.unif.func(theta.grp.1.in = theta.val, 
  ...
  validation.test.frame = data.frame(validation.mat[, c(7:8, 10)])
  colnames(validation.test.frame) = c("t.test", "wilcox.test", "LRT.stats")
  ...
  theta.null = as.numeric(validation.mat[, 6])
  ...
  ## LRT critical values
  if (k.val ==0.2) LRT.cutoff = 1.037
  if (k.val ==0.8) LRT.cutoff = 1.1067
  print(mean(validation.test.frame$LRT.stats>LRT.cutoff))
  ...
  write.csv(val.para.grid, 
            paste0("sim_1/sim_1_n_", n.sample.train, ".csv"))
}
