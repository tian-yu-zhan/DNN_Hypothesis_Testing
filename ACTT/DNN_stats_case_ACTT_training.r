...
## values in the ACTT in Section 5.1
## One can directly work on the validation file with the pre-trained DNN
...
## candidate \theta in generating the training data
prop.train.vec = runif(n.ind, min = 0.15, max = 0.8)
...
n.train.H0.itt = 1*10^4 # B_0 under null in the first DNN
n.train.H1.itt = 1*10^4  # B_1 under alternative in the second DNN
...
## the width of 5 comes from the length of vector of t(s) for TS-DNN
...
for (ind in 1:n.ind){
  ...
  prop.grp.1.train = prop.train.vec[ind] ## \theta_1 in group 1
    
  ## approximate base treatment effect by two-sample t-test
  prop.grp.2.train = min(1, qnorm(alpha, 
          sd = sqrt(2*prop.grp.1.train*(1-prop.grp.1.train)/n.sample.train/2), 
                          lower.tail = FALSE)-
      qnorm(0.84, sd = sqrt(2*prop.grp.1.train*(1-prop.grp.1.train)/n.sample.train/2),
...
## fit the first DNN with all training data with the selected DNN structure
...
save(col_mean_sd_two, file = "ACTT/ACTT_scale_parameters")
...
write.csv(DNN.time.out, file = "ACTT/DNN_training_time.csv")
