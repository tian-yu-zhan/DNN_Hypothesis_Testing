...
## of "DNN-LRT" in the first column of Table 2 in Section 4.2
...
n.dim.data.train = 15 # dimension of t(s) inclusing LRT statistics
...
data.train.para = foreach(ind=1:n.ind) %dopar% {  
  ...
  ## difference between \theta_2 and \theta_1
  delta.train = 2
  ...
  data.train.H0 = t(sapply(1:n.train.H0.itt, 
       function(x){get.data.dist.t.LRT.func(df.grp.1.in = df.train, 
...
para.cutoff.fit = foreach(cutoff.ind=1:n.cutoff) %dopar% {
  ...
  data.cutoff.H0 = t(sapply(1:n.test.H0.inner.itt, 
         function(x){get.data.dist.t.LRT.func(df.grp.1.in = df.cutoff, 
...
save(col_mean_sd_two, file = "sim_2_df_LRT/sim_2_scale_parameters")
...
write.csv(DNN.time.out, file = "sim_2_df_LRT/DNN_training_time.csv")
