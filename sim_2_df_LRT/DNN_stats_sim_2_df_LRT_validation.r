...
## R code for validation to reproduce results of "DNN-LRT" in the first column of
...
first.DNN.model = load_model_hdf5("sim_2_df_LRT/sim_2_first_DNN", 
...
second.DNN.model = load_model_hdf5("sim_2_df_LRT/sim_2_second_DNN", 
                                   custom_objects = NULL, compile = TRUE)
## get the DNN weights
...
load(file = "sim_2_df_LRT/sim_2_scale_parameters")
...
for (val.ind in c(1:n.val.ind)){
  ...
  data.val.fit.para = foreach(test.H0.outer.itt=1:n.test.H0.outer.itt) %dopar% {
    ...
    data.val.fit.temp = get.data.dist.t.LRT.func(df.grp.1.in = df.1.val, 
  ...
  validation.frame = data.frame(validation.mat[, c(1:15)])
  ...
  validation.test.frame = data.frame(validation.mat[, c(17:20)])
  ...
  df.null = as.numeric(validation.mat[, c(16)])
  ...
  write.csv(val.para.grid, 
            paste0("sim_2_df_LRT/sim_2_n_", n.sample.train, ".csv"))
}
