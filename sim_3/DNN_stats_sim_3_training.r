...
# mu.train.vec = runif(n.ind, min = -1, max = 2) # mean
mu.train = 0
sd.train.vec = runif(n.ind, min = 0.2, max = 2) # common sd for two-groups
...
# the width of 3 comes from the length of t(s) for TS-DNN
data.train = matrix(NA, nrow = n.ind*n.train.itt, ncol = 3)
...
for (ind in 1:n.ind){
  ...
  # mu.grp.1.train = mu.train.vec[ind] ## \theta_1 in group 1
  mu.grp.1.train = mu.train  
  
  sd.common = sd.train.vec[ind] ## sd of the normal distribution
  
  ## approximate treatment difference with two-sample t-test
  delta.train = qnorm(alpha, sd = sqrt(2*sd.common^2/n.sample.train), lower.tail = FALSE)-
    qnorm(0.8, sd = sqrt(2*sd.common^2/n.sample.train),lower.tail = FALSE)
  
  ## compute \theta_2 in group 2
  mu.grp.2.train = mu.grp.1.train + delta.train

  ## simulate null data
  data.train.H0 = t(sapply(1:n.train.H0.itt, 
       function(x){get.data.t.func(mu.grp.1.in = mu.grp.1.train, 
                                   mu.grp.2.in = mu.grp.1.train, 
                                   sd.in = sd.common,
                                   n.in = n.sample.train,
                                   if.test = FALSE)$data}))
...
## select the DNN structure with the minimum validation error
...
sd.cutoff.vec = sd.train.vec[1:n.cutoff]
...
para.cutoff.fit = foreach(cutoff.ind=1:n.cutoff) %dopar% {
  ...
  library(tibble)
  library(car)
  # mu.cutoff = mu.cutoff.vec[cutoff.ind] ## \theta_{12} under null
  mu.cutoff = mu.train
  sd.cutoff = sd.cutoff.vec[cutoff.ind] ## common sd under null

  ## simulate null data
  data.cutoff.H0 = t(sapply(1:n.test.H0.inner.itt, 
         function(x){get.data.t.func(mu.grp.1.in = mu.cutoff, 
                                     mu.grp.2.in = mu.cutoff, 
                                     sd.in = sd.cutoff,
                                     n.in = n.sample.train,
                                     if.test = FALSE)$data}))
...
data.cutoff.train = data.frame("sd" = sd.cutoff.vec)
...
save(col_mean_sd_two, file = "sim_3/sim_3_scale_parameters")
...
write.csv(DNN.time.out, file = "sim_3/DNN_training_time.csv")
