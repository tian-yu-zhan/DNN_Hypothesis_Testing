...
## values in the scale uniform distributons of "DNN-T2" in the first column of 
...
## the width of 6 is the same with the length of t(s) with an additional T2 as input
...
for (ind in 1:n.ind){
  ...
  data.train.H0 = t(sapply(1:n.train.H0.itt, 
             function(x){get.data.unif.T2.func(theta.grp.1.in = theta.grp.1.train, 
...
para.cutoff.fit = foreach(cutoff.ind=1:n.cutoff) %dopar% {
  ...
  data.cutoff.H0 = t(sapply(1:n.test.H0.inner.itt, 
                 function(x){get.data.unif.T2.func(theta.grp.1.in = theta.cutoff, 
...
save(col_mean_sd_two, file = "sim_1_with_T2/sim_1_scale_parameters")
...
save_model_hdf5(DNN.cal.cutoff.fit$model, 
                "sim_1_with_T2/sim_1_second_DNN", 
                overwrite = TRUE, include_optimizer = TRUE)
...
write.csv(DNN.time.out, file = "sim_1_with_T2/DNN_training_time.csv")
